...
def test_modulus():
    calc = Calculation(operation="Modulus", operand1=Decimal("17"), operand2=Decimal("5"))
    assert calc.result == Decimal("2")


def test_modulus_by_zero():
    with pytest.raises(OperationError, match="Modulus by zero is not allowed"):
        Calculation(operation="Modulus", operand1=Decimal("17"), operand2=Decimal("0"))


def test_int_divide():
    calc = Calculation(operation="IntDivide", operand1=Decimal("17"), operand2=Decimal("5"))
...
def test_int_divide_by_zero():
    with pytest.raises(OperationError, match="Integer division by zero is not allowed"):
        Calculation(operation="IntDivide", operand1=Decimal("17"), operand2=Decimal("0"))
...
def test_absolute_difference():
    calc = Calculation(operation="AbsoluteDifference", operand1=Decimal("9"), operand2=Decimal("-2"))
    assert calc.result == Decimal("11")


def test_absolute_difference_negative_first():
